...
1. 调用 `bun run --cwd apps/web build:utools` 构建前端资源至 `apps/utools/dist`，该构建将自动使用相对路径，确保在 uTools 的 `file://` 协议下能够正常加载。MathJax 从 `node_modules` 复制到构建产物中，Mermaid 作为 npm 模块打包。
...
> 注意：命令执行前请确认已安装 bun 1.2 与 Node.js 22+，并在仓库根目录执行 `bun install` 安装依赖。

## 手动导入调试

1. 运行 `bun run --cwd apps/web build:utools`。
